...
class Asset(models.Model):
    ...
    name = models.CharField(max_length=128, unique=True, verbose_name='资产序列号')  # 不可重复
    business_unit = models.ForeignKey('BussinessUnit', null=True, verbose_name='所属业务线', on_delete=models.SET_NULL())
    ...
    class Meta:
        verbose_name = '资产总表'
        verbose_name_plural = "资产总表"  # verbose_name_plural 英语有单数和复数形式。这个就是模型对象的复数名
        ordering = ['-c_time']
...
class SecurityDevice(models):
    ...
    sub_asset_type = models.CharField(choices=sub_asset_type_choice, default=0, verbose_name='安全设备类型')
    ...
    class Meta:
        ...
        class NetworkDevice(models.Model):
            ...
            def __str__(self):
                return '%s--%s--%s <sn:%s>' % (
                    self.asset.name, self.get_sub_asset_type_display(), self.model, self.asset.sn)
